...
def create_candidate():
    ...
    # 自动判断简历类型
    resume_type = None
    if 'resume_content' in request.files:
        filename = request.files['resume_content'].filename
        if filename.lower().endswith('.pdf'):
            resume_type = 0
        elif filename.lower().endswith('.doc') or filename.lower().endswith('.docx'):
            resume_type = 1
        else:
            resume_type = 0  # 默认按PDF处理
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?)
    ''', (data['position_id'], data['name'], data['email'], resume_type, resume_binary))
    conn.commit()
    conn.close()
    return jsonify({'status': 'success'})
