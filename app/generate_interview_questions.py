...
def get_candidate_info(candidate_id):
    ...
    cursor.execute('''
        SELECT c.id, c.name, c.email,c.resume_type, c.resume_content, c.position_id
        FROM candidates c
        WHERE c.id = ?
    ''', (candidate_id,))
...
# 根据简历内容和岗位信息生成面试问题
def generate_questions(resume_content, resume_type, position_name, requirements, responsibilities):
    ...
    try:
        ...
        if(resume_type==1):
             resume_text = extract_text_from_word(resume_content)
...
def process_pending_interviews():
    ...
    for interview in pending_interviews:
        ...
        # 修正：解包6个字段
        candidate_id, candidate_name, candidate_email, resume_type, resume_content, position_id = candidate
        ...
        # 修正：传递resume_type参数
        questions = generate_questions(resume_content, resume_type, position_name, requirements, responsibilities)
